...
<head>
	...
	<title>Master Data Description</title>
...
<script type="text/javascript">
	...
	function gridrefresh(){
		$.ajax({
		type : 'POST',
		url  : "<?php echo base_url('home/check_refresh/description'); ?>",
		success : function(data){
			if( Number(data.data) > Number(count)){
				count = data.data;
				if( user_id !== data.user_id){
					$('#datagrid-crud-description').datagrid('reload');
				}
			}
		}
		});
	                ...
	                <script type="text/javascript">
						...
						function ToSearch(){
							var description 	= $('#description').textbox('getValue');
								...
								$('#datagrid-crud-description').datagrid('load',{
...
function create_description(){
	...
	url = '<?php echo base_url('description/create_description'); ?>';
}

function update_description(){
	var row = jQuery('#datagrid-crud-description').datagrid('getSelected');
	if(row){
		jQuery('#dialog-form').dialog('open').dialog('setTitle','Update Data');
		jQuery('#form').form('load',row);	
		url = '<?php echo base_url('description/update_description'); ?>';
	}
}
		...
		success: function(result){
			if (result) {
				...
				if( data.status == "success" ){
					...
					jQuery('#datagrid-crud-description').datagrid('reload');
					$.messager.show({
							title:'INFO',
	...
	if (row){
		...
		jQuery.messager.confirm('Confirm','Are you sure you want to remove ?',function(r){
			if (r){
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('description/delete_description') ?>",
					data: {id:row.id_description},
					dataType: "json",
					success:
					function(data){
						if ( data.status == "success") {
        ...
        <!-- #Body -->
        <div data-options="region:'center',title:'Master Data Description'" style="background-color:#D7E4F2;">
		<!-- TABLE UTAMA -->
		<div style="height:80%" bgcolor="#3E6DB9">
		<table id="datagrid-crud-description" class="easyui-datagrid" style="height:100%" url="<?php echo base_url('description/getJsondescription'); ?>" fit="true" toolbar="#toolbar-user" pagination="false" rownumbers="true" fitColumns="false" singleSelect="true" collapsible="true">
			<thead>
				<tr>
...
<div id="toolbar-user">
	...
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" style="width:100px" onclick="create_description()">Add</a>
	<?php } ?>
	<?php if ( $this->permissions->menu($menu_id, 'updated') ) { ?>
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" style="width:100px" onclick="update_description()">Edit</a>
	<?php } ?>
	<?php if ( $this->permissions->menu($menu_id, 'deleted') ) { ?>
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" style="width:100px" onclick="remove_description()">Delete</a>
	<?php } ?>
</div>
<!-- TOOLBAR EOF -->
<!-- Dialog Form -->
<div id="dialog-form" class="easyui-dialog" style="width:550px; height:250px; padding: 10px 20px" modal="true" closed="true" buttons="#dialog-buttons-user">
	...
	<input type="hidden" name="id_description" required="true"/>
...
<div id="dialog-buttons-user">
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="save_description()">Simpan</a>
