...
class Setting_model extends CI_Model {
		...
		{	
			...
			foreach($menu_child->result_array() as $menu)
			{
				...
				$row[] = array(
					...
					'downloaded'=> (!$menu['downloaded']) ? "" : "<input type='checkbox' name='downloaded' onclick='check_click(" . $menu['menu_id'] . ", \"downloaded\", \"" . $group_id . "\")' id='downloaded" . $menu['menu_id'] . "' " . $downloaded . ">",
	...
	public function getJsonUsers()
	{
		...
		$sql = "SELECT * FROM tbl_user WHERE status_id!='99'";
	...
	public function combogrid_user() {
		// $result['total'] = $this->db->get('tbl_user')->num_rows();
		//$this->db->where('groups_roles_id', $id);
		$nmuser = @$_POST['q'];
		//$this->db->where('groups_roles_id', $id);
