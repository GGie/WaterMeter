...
<script type="text/javascript">
	...
	function gridrefresh(){
		$.ajax({
		type : 'POST',
		url  : "<?php echo base_url('home/check_refresh/group'); ?>",
		success : function(data){
			if( Number(data.data) > Number(count)){
				count = data.data;
				if( user_id !== data.user_id){
					$('#datagrid-crud-group').datagrid('reload');
				}
			}
		}
		});
                   ...
                   <!-- JAVASCRIPT DAN FUNCTION -->
	                <script type="text/javascript">
						...
						var description = $('#description').textbox('getValue');
							$('#datagrid-crud-group').datagrid('load',{
								desc: description,
							}); 
						}
...
function create_group(){
	...
	url = '<?php echo base_url('setting/create_groups'); ?>';
}

function update_group(){
	var row = jQuery('#datagrid-crud-group').datagrid('getSelected');
	if(row){
		jQuery('#dialog-form').dialog('open').dialog('setTitle','Update Data');
		//jQuery('#password').form('load',row.user_id);
		jQuery('#form').form('load',row);
		
		url = '<?php echo base_url('setting/update_group'); ?>/' + row.group_id;
	}
}
	...
	jQuery('#form').form('submit',{
		...
		success: function(result){
				...
				if(data.success){
					...
					jQuery('#datagrid-crud-group').datagrid('reload');
					$.messager.show({
							title:'INFO',
...
function remove_user(){
	...
	if (row){
		jQuery.messager.confirm('Confirm','Are you sure you want to remove ?',function(r){
			if (r){
				jQuery.post('<?php echo base_url('setting/delete_groups'); ?>/',{id:row.user_id},function(result){
					if (result.success){
						jQuery('#datagrid-crud-group').datagrid('reload');
					} else {
						jQuery.messager.show({
        ...
        <!-- #Body -->
        <div data-options="region:'center',title:'Master Group'" style="background-color:#D7E4F2;">
		<!-- TABLE UTAMA -->
		<div style="height:80%" bgcolor="#3E6DB9">
		<table id="datagrid-crud-group" class="easyui-datagrid" style="height:100%" url="<?php echo base_url('setting/group'); ?>" fit="false" toolbar="#toolbar-user" pagination="true" rownumbers="true" fitColumns="false" singleSelect="true" collapsible="true">
			<thead>
				<tr>
					<th field="group_id" width="50" sortable="true">ID</th>
					<th field="group_name" width="150" sortable="true">Group</th>
					<th field="desc" width="200" sortable="true">Description</th>
...
<div id="toolbar-user">
	...
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" style="width:100px" onclick="create_group()">Add</a>
	<?php } ?>
	<?php if ( $this->permissions->menu($menu_id, 'updated') ) { ?>
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" style="width:100px" onclick="update_group()">Edit</a>
	<?php } ?>
</div>
<!-- TOOLBAR EOF -->
<!-- Dialog Form -->
<div id="dialog-form" class="easyui-dialog" style="width:450px; height:250px; padding: 10px 20px" modal="true" closed="true" buttons="#dialog-buttons-user">
	<form id="form" method="post" novalidate >
		
		<input type="hidden" name="group_id" required="true" size="30" maxlength="50" />
		<div class="form-item" style="padding-top:4px">
			<div style="float:left;width:130px;align:right">
				<label>Group</label>
			</div>
			<div>
				<input type="text" name="group_name" class="easyui-textbox" id="password" required="true" size="30" maxlength="50" />
			</div>
		</div>
		<div class="form-item" style="padding-top:4px">
			<div style="float:left;width:130px;align:right">
			...
			<div>
				<input type="text" name="desc" class="easyui-textbox" required="true" size="30" maxlength="50" />
			</div>
		</div>
		</div>
	</form>
